...
def _gt_(self, other):
    """Compare if the rank of this card i higher than 'other'"""
...
def shuffle(self):
    ...
    def insert(card_list, card):
        """Inserts a 'card' in the correct spot of the deck 'card_list' """
        i - 0
...
def sort(self):
            ...
            while len(self.cards) > 0:
                # Go through the deck from left to right, insert the dec in the appropriate spot#
                # Take a card from the old deck, insert it into the new one
                self.insert(sorted_deck, self.take())

            # Replace the old (empty) deck with the new sorted ones
